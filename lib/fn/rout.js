...
module.exports = function (request, response) {
	...
	//判断是否是api请求
	var isApi = pathName.indexOf("/api/") > -1;

	//文件类型
	fileName = pathName.substr(pathName.lastIndexOf("/") + 1);
	fileType = fileName.lastIndexOf(".") > -1 ? fileName.substr(fileName.lastIndexOf(".") + 1) : lastHasG ? "html" : "";
	...
	//完整路径
	//process.cwd() 代码运行路径
	var _dir = isApi ? "server" : "www";
	fullPath = process.cwd() + "/project/" + dirName + "/" + _dir + "/" + _url;

	return {
		dirName: dirName, isApi: isApi, pathName: _url, fullPath: fullPath, method: method, fileName: fileName, fileType: fileType
	};
};