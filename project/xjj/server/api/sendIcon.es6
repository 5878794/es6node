...
module.exports = function(data,request,socket,catchData){
	...
	let	zipName = 'appIcon.jpg';
	let zipPath = path.join(__dirname,"/"+zipName);
		...
		request(
			{
				...
				formData:{
					uploadUser:catchData.icon.userId,
					app_icon:fs.createReadStream(zipPath)
				},
				headers: {  //请求头的设置
