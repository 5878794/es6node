...
DEVICE.touchSlideEvent = function () {
	...
	touch.prototype = {
		...
		getStartAndEndPoint: function getStartAndEndPoint() {
...
DEVICE.loading = function () {
	var __loading = function __loading(datas) {
	...
	__loading.prototype = {
		...
		createCanvas: function createCanvas() {
			...
			if (!this.canvas.getContext) {
				console.log("not suppot canvas");return;
			}
		...
		//添加画板
		appendCanvas: function appendCanvas() {
	...
	var a = function a(obj, scale) {
		obj = obj || $("body");
		this.win = $.getDom(obj);

		if (!this.win) {
			console.log("loading param error");return;
		}
	...
	a.prototype = {
		...
		//创建对象
		_createObj: function _createObj() {
		...
		//阻止事件冒泡
		_addEven: function _addEven() {
			var _box = this.div,
			    _this = this;
			_box.addEventListener(device.START_EV, _this.downfn = function (e) {
				e.stopPropagation();e.preventDefault();
			}, false);
			_box.addEventListener(device.MOVE_EV, _this.movefn = function (e) {
				e.stopPropagation();e.preventDefault();
			}, false);
			_box.addEventListener(device.END_EV, _this.endfn = function (e) {
				e.stopPropagation();e.preventDefault();
			}, false);
		},
		//显示
